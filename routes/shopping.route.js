...
router.post("/product/comment", async (req, res) => {
  if (req.body.userid) {
    ...
    const check = await shoppingModel.insertCommentByUserId(
      req.body.content,
      userid,
      req.body.proID
    );
...
router.get("/cart", async (req, res) => {
  if (req.isAuthenticated()) {
    const cartList = await shoppingModel.findAllCartByID(
...
router.post("/getproductAuthen", async (req, res) => {
  ...
  if (req.isAuthenticated()) {
    ...
    const totalProDuctInCart = await shoppingModel.totalProDuctInCartAuthen(
      userid,
      res.locals.billid
    );
    ...
    const totalProDuctInCart = await shoppingModel.totalProDuctInCartGuest(
      list.billid
    );
    res.render("reload-cart", {
      layout: false,
...
router.post("/checkout", protectRoute, async (req, res) => {
  ...
  const changeBill = await shoppingModel.changeMethodBill(
    res.locals.billid,
    entity
  );
...
router.get("/history", protectRoute, async (req, res) => {
  const list = await shoppingModel.findAllOrderByID(
    req.session.passport.user.id
  );
  list.forEach((u) => {
    if (u.SizeID === u.SizeS) {
